...
public class Like extends HttpServlet {
	...
	//	HttpSession session=request.getSession(true);
		try{
            ...
            if(flike==0) {
                    PreparedStatement ps = con.prepareStatement("update channels set flike=1 where fid="+fid);	//check details
                    ps.executeUpdate();
        	   	    //insert data into viewslikes
        	   	    PreparedStatement ps1=con.prepareStatement("update ViewsLikes set liked=true where stu_id="+id+" and fid="+fid);    	//insert value in viewslikes
        	    	ps1.executeUpdate();
            }
                else {
